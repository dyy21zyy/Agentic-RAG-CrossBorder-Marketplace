...
class HybridRetriever:
    ...
    def retrieve(self,query:str,dense_vector:list[float]|None=None,filters:dict|None=None,top_k:int=20,source_types:list[str]|None=None,mode:str="hybrid_rrf",candidate_k:int|None=None):
        ...
        if mode=="dense_only": return self._dense(query,dense_vector,filters,top_k,source_types)
        if mode=="hybrid_rrf":
            return rrf_fusion([self._bm25(query,filters,top_k,source_types), self._dense(query,dense_vector,filters,top_k,source_types)],k=self.rrf_k,top_k=top_k)
        if mode=="hybrid_rerank":
            if self.reranker is None: raise ValueError("Reranker is required for hybrid_rerank retrieval")
            fused=rrf_fusion([self._bm25(query,filters,candidate_k,source_types), self._dense(query,dense_vector,filters,candidate_k,source_types)],k=self.rrf_k,top_k=candidate_k)
