...
class MilvusChunkStore:
    ...
    def dense_search(self,vector:list[float],filters:dict|None=None,top_k:int=20)->list[EvidenceChunk]:
        ...
        if self.is_lite:
            ...
            res=self.collection.search(data=[vector],anns_field="dense_vector",param={"metric_type":self.metric_type,"params":self.search_params},limit=top_k,expr=expr,output_fields=fields)
