...
class AgenticRAG:
    def __init__(self, duckdb_store: Any | None = None, retriever: Any | None = None, embedding_provider: Any | None = None, max_iterations: int = 2, default_top_k: int = 20) -> None:
        self.duckdb_store=duckdb_store; self.retriever=retriever; self.embedding_provider=embedding_provider; self.max_iterations=max_iterations; self.default_top_k=default_top_k
    def _embed(self, query: str): return None if self.embedding_provider is None else self.embedding_provider.embed_query(query)
    def _retrieve(self, query, plan, source_types=None, mode="hybrid_rrf"):
        if self.retriever is None: raise ValueError("Retriever is required for hybrid retrieval")
        vec=self._embed(query)
        return self.retriever.retrieve(query, dense_vector=vec, filters=plan.filters, top_k=plan.top_k, source_types=source_types if source_types is not None else (plan.source_types or None), mode=mode)
    def run(self, query: str) -> AgentState:
        ...
        if plan.retrieval_route=="sql":
            ...
            state.retrieved_evidence=self._retrieve(plan.query, plan); state.add_tool_call("hybrid_retriever", {"source_types":plan.source_types}); state.add_trace("hybrid_retrieval")
        elif plan.retrieval_route=="mixed":
            if self.duckdb_store is not None:
                state.sql_results=SQLRouter(self.duckdb_store).run(plan); state.add_tool_call("sql_router", plan.filters); state.add_trace("sql_lookup")
            state.retrieved_evidence=self._retrieve(plan.query, plan); state.add_tool_call("hybrid_retriever", {"route":"mixed"}); state.add_trace("mixed_hybrid_retrieval")
        elif plan.retrieval_route=="multi_source_risk":
            sources=plan.source_types or ["policy","trademark","patent","litigation"]
            state.retrieved_evidence=self._retrieve(plan.query, plan, source_types=sources); state.add_tool_call("hybrid_retriever", {"source_types":sources}); state.add_trace("multi_source_risk_retrieval")
        ...
        while not ev.is_sufficient and state.iterations < self.max_iterations and self.retriever is not None:
            ...
            for st, fq in zip(ev.missing_source_types, ev.followup_queries):
                ...
                more=self._retrieve(fq, plan, source_types=[st])
                state.retrieved_evidence.extend(more); state.add_tool_call("hybrid_retriever", {"followup_query":fq,"source_types":[st]}); state.add_trace("followup_retrieval")
            ev=evaluate_evidence(plan, state.retrieved_evidence, state.sql_results); state.evidence_gaps=ev.evidence_gaps; state.add_trace("evaluate_evidence")
        state.reranked_evidence=list(state.retrieved_evidence)
        state.answer,state.citations=synthesize_answer(plan, state.sql_results, state.reranked_evidence, state.evidence_gaps); state.add_trace("final_answer")
        return state
