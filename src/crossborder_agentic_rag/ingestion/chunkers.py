...
def chunk_patent(doc: NormalizedDocument) -> list[EvidenceChunk]:
    ...
    for i, (claim_no, text) in enumerate(claims):
        _append(chunks, make_chunk(doc, "patent_claim", f"Patent {patent_id} claim {claim_no}", f"Patent {patent_id} Claim {claim_no}:\n{text}", {"patent_id": patent_id, "claim_number": claim_no, "claim_index": i, "source_file": md.get("source_file"), "source_path": md.get("source_path")}, f"claim-{i}-{claim_no}", parent_id=parent_id, context_path=f"Patent > {patent_id} > Claim {claim_no}", entity_mentions=_entity_mentions(*base_mentions, claim_no)))
