...
def _records(root: ET.Element) -> list[ET.Element]:
    recs = [e for e in root.iter() if local_name(e.tag) in {"case-file", "trademark-case-file", "casefile"}]
    return recs or [root]


def _doc(record: ET.Element, path: Path, ordinal: int) -> NormalizedDocument:
    ...
    goods = find_all_text(record, ["goods-services", "goods_services"])
    classes = find_all_text(record, ["international-code", "nice-class", "nice_classes"])
    ...
    metadata = {
        "serial_number": serial, "registration_number": registration, "word_mark": word,
        "filing_date": filing, "registration_date": reg_date, "goods_services": goods,
    ...
    return NormalizedDocument(f"trademark:{ident}", "trademark", word or serial or registration or ident, "\n".join(p for p in parts if p), metadata)
    ...
    for path in sorted(base.rglob("*.xml")):
        ...
        try:
            ...
            for i, record in enumerate(_records(root)):
                docs.append(_doc(record, path, i))
