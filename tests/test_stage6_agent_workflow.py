...
def test_policy_infringement_question_is_policy_not_risk():
    c=classify_query('What does Temu policy say about trademark infringement?'); assert (c.query_type,c.expected_answer_type,c.retrieval_route,c.source_types)==('policy_question','policy_answer','hybrid',['policy'])
def test_listing_risk_question_is_risk():
    c=classify_query('Can I sell a phone case using the MERCEDES logo on Temu?'); assert c.query_type=='risk_analysis' and c.retrieval_route=='multi_source_risk' and {'policy','trademark'}<=set(c.source_types)
def test_field_lookup_routes_to_sql(): assert classify_query('Which Nice classes does MERCEDES belong to?').retrieval_route=='sql'
def test_semantic_policy_question_routes_to_hybrid(): assert classify_query('What evidence mentions counterfeit policy enforcement?').retrieval_route=='hybrid'
...
def test_evidence_evaluator_policy_requires_policy_evidence():
    ev=evaluate_evidence(plan('What does Temu policy say?'), []); assert not ev.is_sufficient and ev.missing_source_types==['policy']
def test_evidence_evaluator_risk_requires_policy_plus_other_evidence():
    ev=evaluate_evidence(plan('Can I sell MERCEDES logo?'), [chunks()[0]]); assert not ev.is_sufficient and ev.missing_source_types
def test_evidence_evaluator_builds_followup_queries(): assert 'Find policy evidence for:' in build_followup_query('q','policy')
...
def test_answer_policy_has_no_risk_level(): assert 'Risk Level' not in _ans('What does Temu policy say?')[0]
...
def test_answer_citations_are_not_invented():
    ans,c=synthesize_answer(plan('What does Temu policy say?'), [], [chunks()[0]], []); assert c==['[policy:temu-ip:policy_enforcement:0] Temu IP Policy — policy_enforcement']
...
def test_agent_hybrid_route_calls_retriever():
    r=RecordingRetriever(chunks()); AgenticRAG(retriever=r, embedding_provider=RecordingEmbeddingProvider()).run('What does Temu policy say?'); assert r.calls
...
def test_agent_followup_retrieval_for_missing_policy():
    r=RecordingRetriever([chunks()[1]]); st=AgenticRAG(retriever=r,max_iterations=1).run('What does Temu policy say?'); assert st.iterations==1 and 'followup_retrieval' in st.trace
def test_agent_stops_after_max_iterations():
    r=RecordingRetriever([]); st=AgenticRAG(retriever=r,max_iterations=1).run('What does Temu policy say?'); assert st.iterations==1
def test_agent_trace_contains_required_steps():
    st=AgenticRAG(retriever=RecordingRetriever(chunks())).run('What does Temu policy say?'); assert {'normalize_query','classify_query','plan_retrieval','evaluate_evidence','final_answer'}<=set(st.trace)
def test_agent_state_fields_are_populated():
    st=AgenticRAG(retriever=RecordingRetriever(chunks())).run('What does Temu policy say?'); assert st.normalized_query and st.query_type and st.expected_answer_type and st.retrieval_route and st.retrieval_plan and st.answer and st.citations

def test_cli_demo_mode_outputs_json():
    cp=subprocess.run([sys.executable,'scripts/08_run_query_cli.py','What does Temu policy say about trademark infringement?','--chunks-path','tests/fixtures/agent/sample_chunks.jsonl','--demo','--output-json'],cwd=ROOT,text=True,capture_output=True,check=True); assert json.loads(cp.stdout)['demo_mode'] is True
def test_cli_requires_retriever_without_demo():
    cp=subprocess.run([sys.executable,'scripts/08_run_query_cli.py','What does Temu policy say?'],cwd=ROOT,text=True,capture_output=True); assert cp.returncode!=0 and 'No retrieval backend' in cp.stderr
